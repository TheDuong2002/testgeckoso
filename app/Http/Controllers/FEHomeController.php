...
//            ]);
    ...
    // Trang chính sách
    public function chinhSachChiTIet($href, Request $request){
        $item = PageFooter::where('href', $href)
            ->first();

        if (!$item || $item->status > 0) {
            return view('pages.front_end.invalid.page_not_found');
        }
        return view('pages.front_end.info.policy_member',compact('item'));
    }
