...
@section('content')
    ...
    <div class="config-levels-wrapper">
        <div class="fade-in">
            <form action="{{url('admin/level/update')}}" method="post" id="frm-add" accept-charset="UTF-8"
                  autocomplete="off">
                @csrf
                            ...
                            <div class="card-header">
                                ...
                                <select name="level_id" onchange="changeLevel(this)">
                                    @foreach ($items as $ite)
                                        <option <?php if ($level->id == $ite->id): ?>selected="selected"
                                                <?php endif; ?> value="{{$ite->id}}">{{$ite->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                                                        ...
                                                        <div class="form-wrapper">
                                                            <div class="form-check">
                                                                <label
                                                                    ...
                                                                    <input name="{{$row->title}}" class="c-switch-input"
                                                                           type="checkbox"
                                                                           <?php if ($checked): ?>checked="true"<?php endif;
                                                                                                                    ?>>
                                                            ...
                                                            <label
                                                                @if(in_array($row->title, $embeds)) class="required" @endif>{{$row->getDescription()}}</label>
                                                        </div>
                                                    </div>
